        ...
        size_t size, loff_t *loff)
{
	...
	if(     (payload_size >= PAGE_CACHE_SIZE) &&
                ...
                (payload_size % PAGE_CACHE_SIZE == 0))
        {
                ...
                printk("invalid payload size %u (should be multiples of %lu between %lu - %u)\n",
                        payload_size, PAGE_CACHE_SIZE, PAGE_CACHE_SIZE, RPCSVC_MAXPAYLOAD);
...
static ssize_t payload_write(struct file *file, const char __user *input,
	size_t size, loff_t *loff)
{
	const struct qnap_proc_dir_entry *pde = PDE_DATA(file->f_path.dentry->d_inode);
...
//Patch by QNAP: Setup a interface to rewrite payload size
	...
	sn->proc_net_rpc_tcp_payload = proc_create_data(tcp_payload->name, paylod_perm, sn->proc_net_rpc, &tcp_payload_fops, tcp_payload);
	...
	sn->proc_net_rpc_udp_payload = proc_create_data(udp_payload->name, paylod_perm, sn->proc_net_rpc, &udp_payload_fops, udp_payload);
