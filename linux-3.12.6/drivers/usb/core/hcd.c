...
/* 
  * "Set lpm_capable field for LPM capable root hubs" was created under the assumption
  * that all USB host controllers should have USB 3.0 Link PM enabled for all devices under the hosts.
  * Unfortunately, that's not the case.  The xHCI driver relies on knowledge of the host hardware 
  * scheduler to calculate the LPM U1/U2 timeout values, and it only sets lpm_capable to one 
  * for Intel host controllers (that have the XHCI_LPM_SUPPORT quirk set).
  * When LPM is enabled for Genesys hubs, it causes failures with a Sony USB 3.0 hard drive.
  */
