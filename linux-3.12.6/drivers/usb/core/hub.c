...
int usb_device_supports_lpm(struct usb_device *udev)
{
...
#ifndef CONFIG_MACH_QNAPTS   // Benjamin 2014/05/07 for bug #45652
	/* udev is root hub */
	if (!udev->parent)
		return 1;
	...
	/* 
	  * Benjamin 2014/05/07 for bug #45652: 
	  * GENESYS LOGIC cannot support LPM, and will cause the following error:
	  * "Disable of device-initiated U1 failed. Failed to set U2 timeout to 0x0,error code -71"
	  */
		if (le16_to_cpu(udev->descriptor.idVendor) == USB_VENDOR_GENESYS_LOGIC 
				&& le16_to_cpu(udev->descriptor.idProduct) == 0x0617) {
			dev_warn(&udev->dev, "Genesys USB3.0 hub is found, LPM is not supported.\n");
			return 0;
		}        
		return 1;
    }
...
#ifndef CONFIG_MACH_QNAPTS	// Benjamin 20140424 sync from 3.13.6
...
#ifdef CONFIG_MACH_QNAPTS	//	Back porting from 3.17.0, by LIN_WC, for AMD SteppeEagle bug #49678, 
							//	USB 3.0 device can not be recognized USB 3.0 device, when plug in USB 3.0 port
...
#ifdef CONFIG_MACH_QNAPTS // Benjamin 20140424 sync from 3.13.6
	int port1;
#else
	int i;
#endif
...
#ifdef CONFIG_MACH_QNAPTS   // Benjamin 2014/05/07 for bug #45652
...
/*
  ...
  * usb_enable_link_state+0x30/0x320
  * This is caused by commit "usbcore: compare and release one bos descriptor in
  * usb_reset_and_verify_device()".  That commit saves the old BOS descriptor in a local variable, 
  * and sets udev->bos to NULL before calling into usb_unlocked_disable_lpm().
  ...
  * usb_enable_link_state(), which attempts to dereference the bos  descriptor.
  */
...
#if defined(CONFIG_MACH_QNAPTS) && defined(X86_STEPPEEAGLE)
//	Patched from linux kernel 3.14.6 to resolved Bug #52952 by LIN_WC
/*
		...
		int retry_counter)
{
	...
	for (i = 0; i < GET_DESCRIPTOR_TRIES; (++i, msleep(100))) {
		...
		//	printk("%s:%s:%d, product=%s, devpath=%s, speed=%d\n", __FILE__, __func__, __LINE__, hdev->product, hdev->devpath, hdev->speed);
...
#if defined(CONFIG_MACH_QNAPTS) && defined(X86_STEPPEEAGLE)
			//	Patched from linux kernel 3.14.6 to resolved Bug #52807 by LIN_WC
			if (connect_change){

				int isUX800Usb2Hub = 0;

				if (i == 3 || i == 4)
					if (hub->hdev && hub->hdev->speed == USB_SPEED_HIGH)
						if (hub->hdev->product && strcmp(hub->hdev->product ,"UX-800P") == 0){

							isUX800Usb2Hub = 1;
							//	printk("%s:%s:%d, product=%s, devpath=%s, speed=%d\n", __FILE__, __func__, __LINE__, hub->hdev->product, hub->hdev->devpath, hub->hdev->speed);
						}
