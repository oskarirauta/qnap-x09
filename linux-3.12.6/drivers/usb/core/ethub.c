...
hub_ioctl(struct usb_interface *intf, unsigned int code, void *user_data)
{
	...
	case USBDEVFS_HUB_PORTINFO: {
        ...
        break;
//Patch by QNAP: Support to reset reset USB HUB
	...
	case USBDEVFS_HUB_PORT_TEST: {				
        ...
        break;
	case USBDEVFS_HUB_RESET: {				
        struct usb_hub *hub = hdev_to_hub(hdev);
        int ret;
        if(hub == NULL)
            return -EINVAL;
        printk ("In %s(%d): Run USB HUB reset start\n", __func__, __LINE__);
        ret = ethub_usb_reset_device(hdev);
        printk ("In %s(%d): Run USB HUB reset red, ret = %d\n", __func__, __LINE__, ret);
        return 0;
        }
        break;        
#endif        
	...
	while (1) {
		...
		for (i = 1; i <= hub->descriptor->bNbrPorts; i++) {
			...
			if (hub_port_warm_reset_required(hub, portstatus)) {
				...
				if (!udev || hub_is_superspeed(udev)) {
