...
//Patch by QNAP: for raid1 parallel functionality ==start==
static int RAID1_PARALLEL_INIT_SECTORS = 8192; //Default raid1 parallel_sectors value 

static ssize_t raid1_store_parallel_sectors(struct mddev *mddev, const char *page, size_t len)
{
	struct r1conf *conf = mddev->private;
	unsigned long parallel_sectors;
	...
	if(kstrtoul(page, 10, &parallel_sectors))
		return -EINVAL;

	if ((parallel_sectors % 512) == 0) {
		conf->parallel_sectors = (int)parallel_sectors;
	} else {
		printk(KERN_INFO "The parallel_sectors %lu must be mutilple of 512\n", parallel_sectors);
		return -EINVAL;
	}
...
static ssize_t
raid1_show_parallel_sectors(struct mddev *mddev, char *page)
{
	struct r1conf *conf = mddev->private;

	if (conf)
		return sprintf(page, "%d\n", conf->parallel_sectors);
	else
		return -ENODEV;
}

static struct md_sysfs_entry
raid1_parallel_sectors = __ATTR(parallel_sectors, S_IRUGO | S_IWUSR,
				raid1_show_parallel_sectors,
				raid1_store_parallel_sectors);
static struct attribute *raid1_attrs[] =  {
	&raid1_parallel_sectors.attr,
	NULL,
};
...
//Patch by QNAP: for raid1 parallel functionality ==end==
 ...
 retry:
	...
	for (disk = 0 ; disk < conf->raid_disks * 2 ; disk++) {
		...
		if (conf->mirrors[disk].next_seq_sect == this_sector
			 ...
			 */
			if (conf->parallel_sectors == 0) { //Patch by QNAP: for raid1 parallel functionality - raid1 without parallel read when parallel_sectors = 0
				...
				int opt_iosize = conf->parallel_sectors; //Patch by QNAP: for raid1 parallel functionality
				default_disk = disk;
				if (conf->raid_disks > 1 && opt_iosize > 0 &&
				    mirror->seq_start != MaxSector &&
				    mirror->next_seq_sect > opt_iosize &&
				    mirror->next_seq_sect - opt_iosize >=
				    mirror->seq_start) {
	...
	if (best_disk == -1) {
		if (conf->parallel_sectors == 0) { //Patch by QNAP: for raid1 parallel functionality - raid1 without parallel read when parallel_sectors = 0
			...
			if (best_pending_disk != -1)  //Patch by QNAP: for raid1 parallel functionality - always choose the best_pending_disk
				best_disk = best_pending_disk;
			else
				best_disk = default_disk;
		}
	}
...
//Patch by QNAP: enhance HAL error handler
				...
				((ts_mdname[2]-'0') >= 0 && (ts_mdname[2]-'0') < ts_max_mddata))
		{
			//type = "recover";
			int i;
			printk("raid1: some error occurred in a active device:%d of %s.\n", rdev->raid_disk, ts_mdname);
			sprintf(dev_name,"/dev/%s", bdevname(rdev->bdev,b));
			pFile = filp_open(dev_name,O_RDWR,0);
			...
			for (i = 0; i < conf->raid_disks; i++) {
				struct md_rdev *rdev_tmp = conf->mirrors[i].rdev;
				if (!test_bit(In_sync, &rdev_tmp->flags) && !test_bit(Faulty, &rdev_tmp->flags)){
					printk("raid1(%s): Disk r/w failure on %s, need to stop recovery.\n",
					ts_mdname, bdevname(rdev->bdev,b));					
...
#else
{
            	char ts_devname[4];
            	int ts_caseno;
            	strncpy(ts_devname, bdevname(bio->bi_bdev,b), sizeof(ts_devname));
            	ts_caseno = ts_devname[2]-'a';
...
static struct r1conf *setup_conf(struct mddev *mddev)
{
	...
	//Patch by QNAP: for raid1 parallel functionality
	conf->parallel_sectors = RAID1_PARALLEL_INIT_SECTORS;
...
static int run(struct mddev *mddev)
{
	...
	if (mddev->to_remove == &raid1_attrs_group)
		...
		printk(KERN_WARNING
			"raid1: failed to create sysfs attributes for %s\n",
			mdname(mddev));
	//Patch by QNAP: for raid1 parallel functionality ==end==
...
static int stop(struct mddev *mddev)
{
	...
	kfree(conf);
	mddev->private = NULL;
	mddev->to_remove = &raid1_attrs_group;//Patch by QNAP: for raid1 parallel functionality
	return 0;
}
