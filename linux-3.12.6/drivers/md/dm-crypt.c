...
static int crypt_map(struct dm_target *ti, struct bio *bio)
{
	 ...
	 */
	if (unlikely(bio->bi_rw & (REQ_FLUSH | REQ_DISCARD | REQ_QNAP_MAP))) {
	...
	WARN_ON(bio->bi_rw & REQ_QNAP_MAP_ZERO);
...
static int crypt_locate_thin(struct dm_target *ti, locate_thin_callout_fn fn, sector_t start, sector_t len, void *remap_desc, void **thin)
{
    struct crypt_config *cc = ti->private;

    if (remap_desc)
        ((THIN_REMAP_DESC *)remap_desc)->crypt = 1;

    return fn(cc->dev->bdev, cc->start + dm_target_offset(ti, start), len, remap_desc, thin);
}

static int crypt_invalidate(struct dm_target *ti, sector_t start, sector_t len, invalidate_callback_fn fn, void *data)
{
    struct crypt_config *cc = ti->private;

	return fn(data, cc->dev->bdev, cc->start + dm_target_offset(ti, start));
}
...
static struct target_type crypt_target = {
	...
	.locate_thin = crypt_locate_thin,
	.invalidate = crypt_invalidate,
};
