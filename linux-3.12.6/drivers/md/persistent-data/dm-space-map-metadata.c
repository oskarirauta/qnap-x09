...
{
	int retry = 0, r = -ENOSPC, r2 = 0;
	enum allocation_event ev;
	struct sm_metadata *smm = container_of(sm, struct sm_metadata, sm);

	if (smm->reset_begin <= smm->begin)
		r = sm_ll_find_free_block(&smm->old_ll, smm->begin, smm->old_ll.nr_blocks, b);

	if (r == -ENOSPC) {
		dm_block_t start = (smm->begin < smm->reset_begin) ? smm->begin : 0;
		r = sm_ll_find_free_block(&smm->old_ll, start, smm->reset_begin, b);
		BUG_ON(!r && *b == smm->reset_begin);
	}
