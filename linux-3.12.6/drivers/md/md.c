...
//Patch by QNAP: doing for qnap bbm stripe error in rebuilding mode
static struct workqueue_struct *md_qnap_bbm_wq;
...
static int __qnap_set_readonly(struct mddev *mddev);
static void qnap_set_readonly(struct work_struct *ws);
...
static void md_qnap_badblock_final_check(struct mddev *mddev);
...
void mddev_init(struct mddev *mddev)
{
    ...
    atomic_set(&mddev->qnap_bbm_stripe_error, 0); //Patch by QNAP: init the qnap_bbm_stripe_error flag
    atomic_set(&mddev->qnap_bbm_readonly, 0); //Patch by QNAP: init the qnap_bbm_readonly flag
}
...
static struct md_personality *qnap_find_pers(int level, char *clevel, struct mddev *mddev)
{
	...
	list_for_each_entry(pers, &pers_list, list) {
		if (level != LEVEL_NONE && pers->level == level){
			...
			if(test_bit(QMD_FEATURE_FLEX_RAID, &mddev->qflags) !=
					test_bit(QMD_RAID456_FEATURE_ZONE, &pers->qnap_raid_feature))
				continue;
...
//patch by QNAP: set QNAP_NoDev flag when detecting the no device error
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_USE_STRIPE_ZONE)
    if(le32_to_cpu(sb->qnap_feature) & (1 << eQNAP_MD_SUPER1_FEATURE_FLEX_RAID)){
        set_bit(QMD_FEATURE_FLEX_RAID, &rdev->qflags);
    }
	rdev->sb_size = MD_SB_BYTES;    
#else
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_USE_STRIPE_ZONE)
    int i;
    if(mddev->level>=4 && mddev->level<=6 )
	for (i=0;i<MD_MAX_DISKS;i++){
//		mddev->work_sectors[i] = le64_to_cpu(sb->work_sectors[i]);
		mddev->work_sectors[i] = le64_to_cpu(0);
     }
#endif
	if (mddev->level != LEVEL_MULTIPATH) {
		int role;
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_USE_STRIPE_ZONE)
    if(test_bit(QMD_FEATURE_FLEX_RAID, &mddev->qflags))
        sb->qnap_feature = cpu_to_le32(1<<eQNAP_MD_SUPER1_FEATURE_FLEX_RAID);
#endif
...
retry:
    ...
    if(mddev->level >=4 && mddev->level<=6)
	for (i=0;i<MD_MAX_DISKS;i++){
		sb->work_sectors[i] = cpu_to_le64(mddev->work_sectors[i]);
    }    
#endif
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_USE_STRIPE_ZONE)
//  mddev->is_flexRaid = rdev->is_flexRaid;
    if(test_bit(QMD_FEATURE_FLEX_RAID, &rdev->qflags))
        set_bit(QMD_FEATURE_FLEX_RAID, &mddev->qflags);
#endif
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_USE_STRIPE_ZONE)
	pers = qnap_find_pers(level, clevel, mddev);
#else
	...
	case clean:
		if (mddev->pers) {
			...
			if (atomic_read(&mddev->writes_pending) == 0) {
				...
				err = 0;
				atomic_set(&mddev->qnap_bbm_stripe_error, 0);
				atomic_set(&mddev->qnap_bbm_readonly, 0);
			} else
				err = -EBUSY;
			spin_unlock_irq(&mddev->write_lock);
		} else
...
#ifdef CONFIG_MACH_QNAPTS
static ssize_t
qnap_bbm_show(struct mddev *mddev, char *page) {
    return sprintf(page, "%d\n", test_bit(QMD_FEATURE_BBM, &mddev->qflags));
}
static struct md_sysfs_entry md_qnap_bbm = __ATTR_RO(qnap_bbm);

static ssize_t
qnap_trim_show(struct mddev *mddev, char *page) {
    return sprintf(page, "%d\n",
			blk_queue_discard(mddev->queue) &&
			test_bit(QMD_FEATURE_TRIM, &mddev->qflags));
}
static struct md_sysfs_entry md_qnap_trim = __ATTR_RO(qnap_trim);
#endif
...
int md_run(struct mddev *mddev)
{
	...
	struct md_rdev *rdev;
	struct md_personality *pers;
	bool is_qnap_bblog_enable=true;
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_USE_STRIPE_ZONE)
	pers = qnap_find_pers(mddev->level, mddev->clevel, mddev);
#else
	pers = find_pers(mddev->level, mddev->clevel);
#endif

#if defined (CONFIG_MACH_QNAPTS) && (QNAP_HAL)
	if(mddev->level==4 || mddev->level==5 || mddev->level==6)
	{
		/*Patch by QANP; default disable ssd trim feature in raid456*/
		clear_bit(QMD_FEATURE_TRIM, &mddev->qflags);

		rdev_for_each(rdev, mddev)
		{
			if(rdev->badblocks.shift < 0)
				is_qnap_bblog_enable = false;
		}

		if(is_qnap_bblog_enable)
    		set_bit(QMD_FEATURE_BBM, &mddev->qflags);
		else{
			printk(KERN_INFO "md: %s: the bad block log is disabled in disk superblock.\n",mdname(mddev));
			clear_bit(QMD_FEATURE_BBM, &mddev->qflags);
		}
	} else {
...
//Patch_start by QNAP: [Bad Block Management]
//Set read-only to ensure the rescue action and protect the data.
...
static void qnap_set_readonly(struct work_struct *ws)
{
	struct mddev *mddev = container_of(ws, struct mddev, qnap_bbm_work);
	int err = -1;

	pr_info("md/raid:%s: enter qnap_set_readonly work func...\n", mdname(mddev));

	if (mddev == NULL)
		goto out;

	err = __qnap_set_readonly(mddev);

	if(!err)
		pr_info("md/raid:%s: set md as read-only mode successfully.\n", mdname(mddev));
	else
		pr_info("md/raid:%s: set md as read-only mode fail.\n", mdname(mddev));

out:
	pr_info("md/raid:%s: exit qnap_set_readonly work func...\n", mdname(mddev));
}
#endif
//Patch_end by QNAP: [Bad Block Management]
...
////////////////////////////////////////////////////
	...
	//Patch by QNAP: doing for qnap bbm stripe error in rebuilding mode
	if (atomic_read(&mddev->qnap_bbm_stripe_error) ||
		atomic_read(&mddev->qnap_bbm_readonly))
		return;
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_USE_STRIPE_ZONE)
        if(test_bit(QMD_FEATURE_FLEX_RAID, &mddev->qflags))
        max_sectors = ( mddev->dev_sectors != mddev->resync_max_sectors && mddev->resync_max_sectors )?
			mddev->resync_max_sectors : mddev->dev_sectors;
        else
#endif
		max_sectors = mddev->dev_sectors;
...
//Patch by QNAP: Robust RAID - ReadOnly function
	...
	if ( j !=  MaxSector && (test_bit(MD_RECOVERY_RUNNING, &mddev->recovery) || test_bit(MD_RECOVERY_NEEDED, &mddev->recovery))){
		if (test_bit(MD_RECOVERY_RESHAPE, &mddev->recovery)){
			...
			hal_event.arg.action = RESHAPING_START;
...
//Patch by QNAP: Robust RAID - ReadOnly function
	...
	if ( j != MaxSector && (test_bit(MD_RECOVERY_RUNNING, &mddev->recovery) || test_bit(MD_RECOVERY_NEEDED, &mddev->recovery))){
		if (test_bit(MD_RECOVERY_RESHAPE, &mddev->recovery)){
			...
			hal_event.arg.action = RESHAPING_COMPLETE;
			hal_event.arg.param.netlink_raid.raid_id = simple_strtol(mdname(mddev) + strlen("md"), NULL, 0);
			send_hal_netlink(&hal_event);
		}
...
//Patch by QNAP: Robust RAID - ReadOnly function
	...
	if (test_bit(MD_RECOVERY_INTR, &mddev->recovery)){
		if (test_bit(MD_RECOVERY_RESHAPE, &mddev->recovery)){
			...
			hal_event.arg.action = RESHAPING_SKIP;
			hal_event.arg.param.netlink_raid.raid_id = simple_strtol(mdname(mddev) + strlen("md"), NULL, 0);
			send_hal_netlink(&hal_event);
		}
		else if (test_bit(MD_RECOVERY_SYNC, &mddev->recovery) || test_bit(MD_RECOVERY_REQUESTED, &mddev->recovery)) {
...
/////////////////////////////////////////////////////////////////
 ...
 interrupted:
	...
	//Patch by QNAP: doing for qnap bbm stripe error in rebuilding mode
    if(atomic_read(&mddev->qnap_bbm_stripe_error) == 1)
		    atomic_set(&mddev->recovery_active, 0);
...
void md_reap_sync_thread(struct mddev *mddev)
{
   ...
   md_qnap_badblock_final_check(mddev);
#endif
			    ...
			    int acknowledged)
{
	...
	bb->prev_count=bb->count; //Patch by QNAP: reset bb->prev_count
...
static void md_qnap_bbm_stripe_error(struct work_struct *ws)
{
	struct mddev *mddev = container_of(ws, struct mddev, qnap_bbm_work);

	printk(KERN_ERR "md/raid:%s: enter qnap_bbm_work ...\n",mdname(mddev));		

	if (mddev->sync_thread) {
		printk(KERN_ERR "md/raid:%s: in qnap_bbm_work, try to unregister sync thread and do the related handling process...\n",mdname(mddev));		
		md_reap_sync_thread(mddev);
	}

	printk(KERN_ERR "md/raid:%s: exit qnap_bbm_work ...\n",mdname(mddev));		
}    
...
int qnap_badblock_handler(struct md_rdev *rdev, sector_t s, int sectors)
{
	...
	if (mddev->ro == 1 ||
	    atomic_read(&mddev->qnap_bbm_stripe_error) == 1 ||
	    atomic_read(&mddev->qnap_bbm_readonly) == 1)
		return 0;
	...
	if(rdev->badblocks.prev_count==0) //bug53503: avoid reporting too many badblock logs
	...
	if(rv2){
		...
		//report stripe error event
		md_qnap_badblock_event(BAD_BLOCK_ERROR_STRIPE, rdev, first_bad, bad_sectors, bad_count);

		// QNAP BBM State #6: set raid to read only mode
		if (mddev->pers) {

			//If it is not in raid rebuilding mode, set read-only directly.
			if(!test_bit(MD_RECOVERY_RUNNING, &mddev->recovery)){

				printk(KERN_ERR "md/raid:%s: md_qnap_is_stripe_error. Not in rebuilding mode, so set md as read-only mode directly.\n",
						mdname(mddev));		

				if (atomic_read(&mddev->qnap_bbm_readonly) == 0) {

					atomic_set(&mddev->qnap_bbm_readonly, 1);

					printk(KERN_ERR "md/raid:%s: init qnap_bbm_work, do qnap_set_readonly process...\n",mdname(mddev));
					INIT_WORK(&mddev->qnap_bbm_work, qnap_set_readonly);
					queue_work(md_qnap_bbm_wq, &mddev->qnap_bbm_work);
				}
				else
					printk(KERN_ERR "md/raid:%s: qnap_bbm thread have been already exist...\n",mdname(mddev));
			}
			else
			{
				printk(KERN_ERR "md/raid:%s: md_qnap_is_stripe_error. In rebuilding mode, so start a qnap_bbm thread and set md as read-only mode later.\n",
						mdname(mddev));		

				//If it is in raid rebuilding mode, start a qnap_bbm work to handle the sync thread interrupt event first.
				//Finally, call md_reap_sync_thread() to unregister the sync thread and do the related handling function.
				if(atomic_read(&mddev->qnap_bbm_stripe_error)==0){

					atomic_set(&mddev->qnap_bbm_stripe_error, 1);

					printk(KERN_ERR "md/raid:%s: init qnap_bbm_work, do md_qnap_bbm_stripe_error process...\n",mdname(mddev));	
					INIT_WORK(&mddev->qnap_bbm_work, md_qnap_bbm_stripe_error);
					queue_work(md_qnap_bbm_wq, &mddev->qnap_bbm_work);
				}
				else
					printk(KERN_ERR "md/raid:%s: qnap_bbm thread have been already exist...\n",mdname(mddev));
	...
	if(rv3){
		...
		// QNAP BBM State #5 and #8 will be handled by 
		// md_qnap_badblock_final_check() after the recovery process is finished
		if(!isExistSpare){

			printk(KERN_ERR "md/raid:%s: md_qnap_is_multiple_badblocks, set md as read-only mode.\n",
					mdname(mddev));

			// QNAP BBM State #7: set raid to read only mode
			if (mddev->pers) {
	...
	if(!isExistSpare){
		// QNAP BBM State #1: enter to degrade mode 
		md_error(mddev, rdev);
	}
...
int md_qnap_is_multiple_badblocks(struct md_rdev *rdev)
{
	...
	//If it has been already in degraded mode, it must be set read-only when detecting extra bad block.
    if ( (mddev->degraded == 1 && level == 4 ) ||
         (mddev->degraded == 1 && level == 5 ) ||
         (mddev->degraded == 2 && level == 6 ) )
	{
		ret = 1;
		return ret;
	}
	...
	rdev_for_each_rcu(check_rdev,mddev){
		...
		//ignore the faulty device if the md is not in degraded mode
                if((mddev->degraded == 0) && test_bit(Faulty, &check_rdev->flags))
                        continue;
...
/***************************************************
 * md_qnap_badblock_final_check:
 * describe:
 * 	Handle the QNAP BBM State #5 and #8
 *	After the recovery process is finished,
 *	we should check if there is any bad block be 
 *	detected during the recovery process.
 *	If yes, we should let the array enter the 
 *	degrade mode (single bad block) or 
 *	read-only mode (multiple bad blocks).
 **************************************************/
static
void md_qnap_badblock_final_check(struct mddev *mddev)
{
	struct md_rdev *rdev;
	char b[BDEVNAME_SIZE];	
	int spare=0;
	...
	//handle the stripe error in rebuilding mode
	if(atomic_read(&mddev->qnap_bbm_stripe_error)==1)
	{
		...
		printk(KERN_INFO "md: %s: handle the bbm stripe error in rebuilding mode...\n",mdname(mddev));

		rdev_for_each(rdev, mddev){

			if(test_bit(Replacement, &rdev->flags)){

				printk(KERN_INFO "md: %s: (md:%s,dev:%s): roll-back Replacement disk. id= %d, flags = 0x%x\n",
						__func__,mdname(mddev),bdevname(rdev->bdev,b),rdev->raid_disk,rdev->flags);

				repl_disk = rdev->raid_disk;
		...
		rdev_for_each(rdev, mddev){
			...
			if(test_bit(WantReplacement, &rdev->flags) && repl_disk == rdev->raid_disk)
			{
				printk(KERN_INFO "md: %s: (md:%s,dev:%s): fail WantReplacement disk. id= %d, flags = 0x%x\n",
						__func__,mdname(mddev),bdevname(rdev->bdev,b),rdev->raid_disk,rdev->flags);
	...
	//count the number of the spare disk in the array
	rdev_for_each(rdev, mddev){

		if((rdev->raid_disk == -1) && (rdev->flags == 0)){
			spare++;

			printk(KERN_INFO "md: %s: (md:%s,dev:%s): spare disk get. id= %d, flags = 0x%x, spare = %d\n",
					__func__,mdname(mddev),bdevname(rdev->bdev,b),rdev->raid_disk,rdev->flags,spare);
		}
	}

	rdev_for_each(rdev, mddev){

		printk(KERN_INFO "md: %s: (md:%s,dev:%s): id= %d, flags = 0x%x, bb count = %d\n",
				__func__,mdname(mddev),bdevname(rdev->bdev,b),rdev->raid_disk,rdev->flags,rdev->badblocks.count);
		...
		//If there is a spare disk for the disk with bad block, do not set fault to this disk.
		//The md_check_recovery() of raid5d will do the hot-replace process for it automatically.
		//On the contrary, just set error to the fault disk without spare disk.
		if((--spare) < 0 ||
		    atomic_read(&mddev->qnap_bbm_stripe_error) == 1)
			md_error(mddev, rdev);
	}
...
void md_qnap_badblock_event(unsigned int err, struct md_rdev *rdev, sector_t sector, int len, unsigned int count)
{
	...
	printk("md/raid:%s: report badblock event: case %d, pd_name=/dev/%s, sector=%lu, len=%d, count=%u\n",
							mdname(mddev),err,devname,sector,len,count); 
	...
	switch(err){
		...
		case BAD_BLOCK_ERROR_STRIPE:
			...
			hal_event.arg.param.badblock.raid_id = simple_strtol(mdname(mddev) + strlen("md"), NULL, 0);
...
static struct notifier_block md_notifier = {
	...
	.next		= NULL,
#ifdef CONFIG_MACH_QNAPTS
	.priority	= INT_MAX-1, /* before any real devices but after flashcache */
#else
...
static int __init md_init(void)
{
	...
	//Patch by QNAP: doing for qnap bbm stripe error in rebuilding mode
	md_qnap_bbm_wq = alloc_workqueue("md_qnap_bbm", 0, 0);
	if (!md_qnap_bbm_wq){
		printk(KERN_ERR "md: could not alloc md_qnap_bbm workqueue...\n");
		goto err_wq;
	}
...
static __exit void md_exit(void)
{
	...
	//Patch by QNAP: doing for qnap bbm stripe error in rebuilding mode
	destroy_workqueue(md_qnap_bbm_wq);
