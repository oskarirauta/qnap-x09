...
#ifdef CONFIG_MACH_QNAPTS
#define DIR_NODE    "fintek_cir"
#define FILE_NODE   "cir_enable"

static struct fintek_dev s_fintek;
static int s_cir_enable = 0;
static struct proc_dir_entry *s_fintek_proc_dir, *s_cir_enable_file;
//static spinlock_t pcoc_fs_lock;
#endif
...
#ifdef CONFIG_MACH_QNAPTS
static void fintek_cir_regs_init_with_irq_enable(struct fintek_dev *fintek)
{
	/* clear any and all stray interrupts */
	fintek_cir_reg_write(fintek, CIR_STATUS_IRQ_MASK, CIR_STATUS);
...
//Patch by QNAP: fix RX fail randomly
#ifndef CONFIG_MACH_QNAPTS
		fintek_cir_reg_write(fintek, CIR_STATUS_IRQ_MASK, CIR_STATUS);
#endif        
...
#ifdef CONFIG_MACH_QNAPTS
static struct fintek_dev *fintek;
#endif
...
static int fintek_resume(struct pnp_dev *pdev)
{
	...
	fintek_cir_regs_init(fintek);
#else
	fintek_cir_regs_init_with_irq_enable(fintek);
#endif
...
#ifdef CONFIG_MACH_QNAPTS
// Benjamin 20140416:Use proc_create instead of create_proc_entry. FIXME: read op
#if 0
static int fintek_proc_read(char *page, char **start, off_t off, int count,
                            int *eof, void *data)
{
    *eof = 1;
    cir_dump_regs(&s_fintek);
    return sprintf(page, "%d\n", s_cir_enable);
}
#endif

static ssize_t fintek_proc_write(struct file *file, const char __user *buffer,
                              size_t count, loff_t *pos)
{
    ...
    if(s_cir_enable == 1) {
        ...
        do {
            data = fintek_cir_reg_read(fintek, CIR_RX_DATA);
        } while(data != 0xff);
...
static int fintek_proc_show(struct seq_file *m, void *v) 
{
    cir_dump_regs(&s_fintek);
...
static int fintek_proc_open(struct inode *inode, struct  file *file) {
  return single_open(file, fintek_proc_show, NULL);
}

static const struct file_operations fintek_proc_fops = {
        .owner          = THIS_MODULE,
        .open           = fintek_proc_open,
        .read           = seq_read,
        .write         = fintek_proc_write,
...
{
#ifdef CONFIG_MACH_QNAPTS
    s_fintek_proc_dir = proc_mkdir(DIR_NODE, NULL);
    if(!s_fintek_proc_dir) {
        printk("[%s] failed to mkdir \"%s\" in procfs \n", __FUNCTION__, DIR_NODE);
        return -1;
    }
    
    s_cir_enable_file = proc_create(FILE_NODE, S_IRUGO | S_IWUSR, s_fintek_proc_dir, &fintek_proc_fops);
    
    if(!s_cir_enable_file) {
        printk("[%s] failed to create \"%s\" in procfs \n", __FUNCTION__, FILE_NODE);
        remove_proc_entry(DIR_NODE, NULL);
        return -1;
    }    
#endif    
