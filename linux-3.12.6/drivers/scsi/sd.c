...
#if defined(CONFIG_MACH_QNAPTS)
				unsigned long granularity_offset = find_first_bit((unsigned long *)&sdkp->unmap_granularity, 32);

				printk("Modify BLV from SAS: sdkp->unmap_granularity=%x, OPTIMAL TRANSFER LENGTH=%x, lba_count=%x, desc_count=%x\n",sdkp->unmap_granularity, get_unaligned_be32(&buffer[12]), lba_count ,desc_count);
				if ((sdkp->unmap_granularity >> (granularity_offset + 1)) > 0) {
					blk_queue_io_min(sdkp->disk->queue, 1 * sector_sz);
