...
static atomic_t max_wakeup_pds[MAX_ENCLOSURE_ID];
...
static void add_to_hal_container(HAL_NETLINK_CB* netlink_cb)
{
    ...
    {
        old_netlink_cb = list_entry(hal_container_head.next, HAL_NETLINK_CB, node);
...
static void hal_netlink_func(struct work_struct *work)
{
    ...
    if (hal_netlink != NULL)
    {
        ...
        else
        {
            ...
            ret = netlink_broadcast(hal_netlink,skb, 0, 5, GFP_KERNEL);
            if (ret < 0)
            {
//                printk("netlink_broadcast fail = %d\n",ret);
...
int send_hal_netlink(NETLINK_EVT *hal_event)
{
    ...
    if (netlink_cb == NULL)
    {
        printk("%s fail:memory allocate fail\n",__func__);
        return -1;
    }
...
//Patch by QNAP: only for HAL application
        ...
        struct scsi_device *sdev = cmd->device;
        if (sdev->is_standby == 1)
        {
//            printk("ENC:HOST=%d:%d  is in standby mode, current allow standby disks = %d,cmd = 0x%x\n",sdev->hal_enc_id,sdev->host->host_no,atomic_read(&max_wakeup_pds[sdev->hal_enc_id]),cmd->cmnd[0]);
            if (atomic_dec_return(&max_wakeup_pds[sdev->hal_enc_id]) >= 0)
            {
                NETLINK_EVT hal_event;
                struct __netlink_pd_cb *netlink_pd;
                sdev->is_standby = 0;
                sdev->is_wakeup_cmd = 1;
                hal_event.type = HAL_EVENT_GENERAL_DISK;
                hal_event.arg.action = CLEAR_PD_STANDBY;
            ...
            {
//                printk("ENC:HOST=%d:%d is in standby mode, return busy\n",sdev->hal_enc_id,sdev->host->host_no);
                atomic_inc(&max_wakeup_pds[sdev->hal_enc_id]);
...
void scsi_finish_command(struct scsi_cmnd *cmd)
{
	...
	good_bytes = scsi_bufflen(cmd);
        ...
        //Patch by QNAP: only for HAL application
#if defined(CONFIG_MACH_QNAPTS) && defined(QNAP_HAL)
    if(sdev->is_wakeup_cmd == 1 && cmd->cmnd[0] != SYNCHRONIZE_CACHE)
    {
        sdev->is_wakeup_cmd = 0;
                atomic_inc(&max_wakeup_pds[sdev->hal_enc_id]);
        //        printk("%s:ENC:host=%d:%d wakeup disk command, restore max_standby_disk = %d\n",__func__,sdev->hal_enc_id,sdev->host->host_no,atomic_read(&max_wakeup_pds[sdev->hal_enc_id]));
            }
#endif
/////////////////////
