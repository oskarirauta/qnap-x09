...
static void fd_free_device(void *p)
{
	...
	if (file->f_mode & FMODE_JOURNAL_SUPPORT) {
	        printk(KERN_ERR "[BLKJBD] release_blkdev_journal\n");
...
#ifdef QNAP_SHARE_JOURNAL
static int fd_set_journal_support(struct se_subsystem_dev *se_sub_dev,
				  unsigned long enable)
{
	int err = 0;
	struct fd_dev *fd_dev = se_sub_dev->se_dev_su_ptr;
	struct file *file = fd_dev->fd_file;
	struct inode *inode = file->f_mapping->host;
