 ...
 * @retval
 */
static ssize_t target_core_show_dev_bbu_journal(void *p, char *page)
{
	struct se_subsystem_dev *se_sub_dev = p;
	ssize_t rb;

	if (!(se_sub_dev->su_dev_flags & SDF_USING_BBUJOURNAL))
		return 0;
	rb = snprintf(page, PAGE_SIZE, "%llu\n",
		      (u64)se_sub_dev->se_dev_bbu_journal);
	return rb;
}
/*
 ...
 * @retval
 */
static ssize_t target_core_store_dev_bbu_journal(void *p, const char *page,
						 size_t count) {
	struct se_subsystem_dev *se_sub_dev = p;
	struct se_hba *hba = se_sub_dev->se_dev_hba;
	struct se_subsystem_api *t = hba->transport;
	...
	ret = strict_strtoul(page, 0, &enable);
	if (ret < 0) {
		pr_err("strict_strtoul() failed with ret: %d\n", ret);
		return -EINVAL;
	}
	...
	se_sub_dev->su_dev_flags |= SDF_USING_BBUJOURNAL;

	if (!(!enable && se_sub_dev->se_dev_bbu_journal)
			&& !(enable && !se_sub_dev->se_dev_bbu_journal)) {
		return count;
	}

	last_status = se_sub_dev->se_dev_bbu_journal;
	se_sub_dev->se_dev_bbu_journal = enable;

	if (t && t->set_journal_support) {
		ret = t->set_journal_support(se_sub_dev, enable);
		if (ret) {
			se_sub_dev->se_dev_bbu_journal = last_status;
			return -EINVAL;
		}
		pr_debug("Target_Core_ConfigFS: se_sub_dev[%p], "
			 "%s/%s configure journal support: %u\n",
			 se_sub_dev,
			 config_item_name(&hba->hba_group.cg_item),
			 config_item_name(&se_sub_dev->se_dev_group.cg_item),
			 se_sub_dev->se_dev_bbu_journal);
	}

	return count;
}

static struct target_core_configfs_attribute target_core_attr_dev_bbu_journal =
{
	.attr = {.ca_owner = THIS_MODULE,
		 .ca_name = "bbu_journal",
		 .ca_mode = S_IRUGO | S_IWUSR},
