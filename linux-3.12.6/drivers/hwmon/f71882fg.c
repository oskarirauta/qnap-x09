...
//Patch by QNAP: add x79  model
#define SIO_F71882FG_LD_GPIO	0x06	/* GPIO logical device */
#define SIO_F71882FG_LD_PME	    0x0a	/* PME,ACPI and Power Saving logical device */
////////////////////////////
#endif
...
#ifdef CONFIG_MACH_QNAPTS
//Patch by QNAP: Support F81866A
#define SIO_F81866A_ID		0x1010	/* Chipset ID */
#endif
...
#ifdef CONFIG_MACH_QNAPTS
//Patch by QNAP: add x79  model
#define F71882FG_REG_TEMP_ALERT_ENABLE1		0x66
////////////////////////////////
#endif
...
#ifdef CONFIG_MACH_QNAPTS
#define F71882FG_REG_FAN_FAULT_TIME	0x9F
#endif
...
enum chips { f71808e, f71808a, f71858fg, f71862fg, f71869, f71869a, f71882fg,
	     f71889fg, f71889ed, f71889a, f8000, f81865f, f81866a };
#else
enum chips { f71808e, f71808a, f71858fg, f71862fg, f71869, f71869a, f71882fg,
...
//Patch by QNAP: add x79  model
//Support to read CPU temp from PECI
#ifdef CONFIG_MACH_QNAPTS
    unsigned short sio_addr;
    u8 peci_cpu_temp;
#endif
...
static ssize_t show_temp(struct device *dev, struct device_attribute
	*devattr, char *buf);
//Patch by QNAP: Support to read CPU temp from PECI
#ifdef CONFIG_MACH_QNAPTS
static ssize_t show_peci_cpu_temp(struct device *dev, struct device_attribute
	*devattr, char *buf);
#endif
...
#ifdef CONFIG_MACH_QNAPTS
#if defined(TS470U) // Support Digital IO Board
/* Sysfs sio input */
static ssize_t show_sio_in(struct device *dev, struct device_attribute *devattr,
        char *buf);
static ssize_t show_sio_in_bitmask(struct device *dev, struct device_attribute *devattr,
        char *buf);
static ssize_t show_sio_out(struct device *dev, struct device_attribute *devattr,
        char *buf);
static ssize_t show_sio_out_bitmask(struct device *dev, struct device_attribute *devattr,
        char *buf);
static ssize_t store_sio_out(struct device *dev, struct device_attribute *devattr,
        const char *buf, size_t count);
#endif

//Patch by QNAP: add case_status attribute in sys
static ssize_t show_case_status(struct device *dev, struct device_attribute *devattr,
	char *buf);
...
static DEVICE_ATTR(case_status, S_IRUGO, show_case_status, NULL);
#endif
...
//Patch by QNAP: Support to read CPU temp from PECI
#ifdef CONFIG_MACH_QNAPTS
static struct sensor_device_attribute_2 peci_cup_temp_attr[] = {
	SENSOR_ATTR_2(peci_cpu_temp, S_IRUGO, show_peci_cpu_temp, NULL, 0, 0),
};
#endif
...
#if defined(TS470U) // Support Digital IO Board
/* SIO INPUT */
static struct sensor_device_attribute_2 fxxxx_sio_input_attr[8] = {
        SENSOR_ATTR_2(sio_input0, S_IRUGO, show_sio_in, NULL, 0, 0),
        SENSOR_ATTR_2(sio_input1, S_IRUGO, show_sio_in, NULL, 0, 1),
        SENSOR_ATTR_2(sio_input2, S_IRUGO, show_sio_in, NULL, 0, 2),
        SENSOR_ATTR_2(sio_input3, S_IRUGO, show_sio_in, NULL, 0, 3),
        SENSOR_ATTR_2(sio_input4, S_IRUGO, show_sio_in, NULL, 0, 4),
        SENSOR_ATTR_2(sio_input5, S_IRUGO, show_sio_in, NULL, 0, 5),
        SENSOR_ATTR_2(sio_input6, S_IRUGO, show_sio_in, NULL, 0, 6),
        SENSOR_ATTR_2(sio_input7, S_IRUGO, show_sio_in, NULL, 0, 7),
};

static struct sensor_device_attribute_2 fxxxx_sio_input_bitmask_attr =
        SENSOR_ATTR_2(sio_input_bitmask, S_IRUGO, show_sio_in_bitmask, NULL, 0, 0);

/* SIO OUTPUT */
static struct sensor_device_attribute_2 fxxxx_sio_output_attr[8] = {
        SENSOR_ATTR_2(sio_output0, S_IRUGO | S_IWUSR, show_sio_out, store_sio_out, 0, 0),
        SENSOR_ATTR_2(sio_output1, S_IRUGO | S_IWUSR, show_sio_out, store_sio_out, 0, 1),
};

static struct sensor_device_attribute_2 fxxxx_sio_output_bitmask_attr =
        SENSOR_ATTR_2(sio_output_bitmask, S_IRUGO, show_sio_out_bitmask, NULL, 0, 0);
...
//Patch by QNAP: BIOS initialize CMOS
#ifdef CONFIG_MACH_QNAPTS
...
//Patch by QNAP: Support to read CPU temp from PECI
#ifdef CONFIG_MACH_QNAPTS
        data->peci_cpu_temp = f71882fg_read_temp(data,0x7);
#endif
///////////////////
	...
	char *buf)
{
	...
	{
//Patch by QNAP: read negative temperature
#if defined(CONFIG_MACH_QNAPTS)
        if (data->temp[nr] > 0x7f)
            temp = (data->temp[nr] - 0x100) * 1000;
        else
...
//Patch by QNAP: Support to read CPU temp from PECI
#ifdef CONFIG_MACH_QNAPTS
static ssize_t show_peci_cpu_temp(struct device *dev, struct device_attribute *devattr,
	char *buf)
{
	struct f71882fg_data *data = f71882fg_update_device(dev);
	return sprintf(buf, "%d\n", data->peci_cpu_temp);
        ...
        char *buf)
{
    ...
    mutex_lock(&data->update_lock);
    superio_enter(data->sio_addr);
    superio_select(data->sio_addr, SIO_F71882FG_LD_GPIO);

    val = superio_inb(data->sio_addr,0xc2);

    superio_exit(data->sio_addr);
    mutex_unlock(&data->update_lock);
        ...
        char *buf)
{
    ...
    val = superio_inb(data->sio_addr,0xb1);
        ...
        const char *buf, size_t count)
{
    ...
    superio_outb(data->sio_addr,0xb1,val);
    superio_exit(data->sio_addr);
    mutex_unlock(&data->update_lock);
...
#endif // defined(TS470U) // Support Digital IO Board
#endif
	...
	char *buf)
{
    ...
    u8 open = 0;
    mutex_lock(&data->update_lock);
    if(f71882fg_read8(data,0x03) & 0x1)
        f71882fg_write8(data,0x03,0x1);
    if(!(f71882fg_read8(data,0x03) & 0x1))
       open = 1; //Cover Open
    mutex_unlock(&data->update_lock);
	return sprintf(buf, "%d\n", open);
...
//Patch by QNAP: add x79  model
#ifdef CONFIG_MACH_QNAPTS
    data->sio_addr = sio_data->sio_addr;
    //Clear FAN_PROG_SEL in F71882FG_REG_FAN_FAULT_TIME
    f71882fg_write8(data,F71882FG_REG_FAN_FAULT_TIME, f71882fg_read8(data, F71882FG_REG_FAN_FAULT_TIME) & ~0x80);
    wmb();
    f71882fg_write8(data,F71882FG_REG_PWM_ENABLE,0x3f);
    f71882fg_write8(data,F71882FG_REG_TEMP_ALERT_ENABLE1,0x0);
    f71882fg_write8(data,F71882FG_REG_TEMP_TYPE,0x0);
#endif
...
//Patch by QNAP: add case_status attribute in sys
#if defined(CONFIG_MACH_QNAPTS) && defined(QNAP_HAL)
    err = device_create_file(&pdev->dev, &dev_attr_case_status);
	if (err)
...
#if defined(TS470U) // Support Digital IO Board
                for (i = 0; i < 8; i++) {
                        err = device_create_file(&pdev->dev, &fxxxx_sio_input_attr[i].dev_attr);
                        if (err)
                                goto exit_unregister_sysfs;
                }
                err = device_create_file(&pdev->dev, &fxxxx_sio_input_bitmask_attr.dev_attr);
                if (err)
                   goto exit_unregister_sysfs;
                for (i = 0; i < 2; i++) {
                        err = device_create_file(&pdev->dev, &fxxxx_sio_output_attr[i].dev_attr);
                        if (err)
                                goto exit_unregister_sysfs;
                }
                err = device_create_file(&pdev->dev, &fxxxx_sio_output_bitmask_attr.dev_attr);
                if (err)
                   goto exit_unregister_sysfs;
#endif
#endif
...
//Patch by QNAP: add case_status attribute in sys
#if defined(QNAP_HAL)
	device_remove_file(&pdev->dev, &dev_attr_case_status);
#endif
#endif
/////////////////////////////////
	if (data->hwmon_dev)
...
#if defined(TS470U)
                for (i = 0; i < 8; i++) {
                   device_remove_file(&pdev->dev, &fxxxx_sio_input_attr[i].dev_attr);
                }
                device_remove_file(&pdev->dev, &fxxxx_sio_input_bitmask_attr.dev_attr);
                for (i = 0; i < 2; i++) {
                   device_remove_file(&pdev->dev, &fxxxx_sio_output_attr[i].dev_attr);
                }
                device_remove_file(&pdev->dev, &fxxxx_sio_output_bitmask_attr.dev_attr);
#endif
#endif
	}
...
//Patch by QNAP: add x79  model
#ifdef CONFIG_MACH_QNAPTS
        sio_data->sio_addr = sioaddr;
        if (sio_data->type == f81866a)
        {
            //Do nothing
        }
        else
        {
#if defined(X86_HASWELL) || defined(X86_BAYTRAIL) || defined(X86_STEPPEEAGLE)
            //Do nothing, let the register values be the same as those set by BIOS.
            // Set GPIO53 high
            superio_outb(sioaddr,0xa0,superio_inb(sioaddr,0xa0) | 0x08);    // set GP53 output
            superio_outb(sioaddr,0xa1,superio_inb(sioaddr,0xa1) | 0x08);    // set GP53 output high
            superio_outb(sioaddr,0xa3,superio_inb(sioaddr,0xa3) | 0x08);    // set GP53 push-pull
#if defined(X86_STEPPEEAGLE)
		superio_outb(sioaddr,0x2a,superio_inb(sioaddr,0x2a) & ~0x03); //keep UART2
	    		superio_select(sioaddr, SIO_F71882FG_LD_GPIO);
		superio_outb(sioaddr,0x29,superio_inb(sioaddr,0x29) & ~0x60); //Enable LED_VCC
#endif
#else
            superio_outb(sioaddr,0x26,superio_inb(sioaddr,0x26) & ~0x08);
            superio_outb(sioaddr,0x29,superio_inb(sioaddr,0x29) | 0xC4); //GP12,GP17,GP16
#if defined(TS270) || defined(TS470) || defined(TS470U) || defined(TS670) || defined(TS870)
#else
            superio_outb(sioaddr,0x2c,superio_inb(sioaddr,0x2c) | 0x08); //GP3
#endif
            superio_outb(sioaddr,0x2a,superio_inb(sioaddr,0x2a) | 0x80); //Pin100-P116 as GPIO
            superio_outb(sioaddr,0x2a,superio_inb(sioaddr,0x2a) & ~0x03); //keep UART2

    		superio_select(sioaddr, SIO_F71882FG_LD_GPIO);
            superio_outb(sioaddr,0x30,superio_inb(sioaddr,0x30) | 0x01); //enable GPIO I/O port

            //Reset Button : GP61
            //Status LED: Green GP62
            //Status LED: Read GP63
            superio_outb(sioaddr,0x90,(superio_inb(sioaddr,0x90) & ~0x2) | 0x0c);
            superio_outb(sioaddr,0x91,superio_inb(sioaddr,0x91) | 0x0c );
            superio_outb(sioaddr,0x93,superio_inb(sioaddr,0x93) | 0x0c);

            //USB LED : GP17
            superio_outb(sioaddr,0xe0,superio_inb(sioaddr,0xe0) | 0x80);
            superio_outb(sioaddr,0xe1,superio_inb(sioaddr,0xe1) | 0x80);
            superio_outb(sioaddr,0xe3,superio_inb(sioaddr,0xe3) | 0x80);

            //USB Button :GP12
            superio_outb(sioaddr,0xe0,superio_inb(sioaddr,0xe0) & ~0x04);

            //Power LED :GP16
            superio_outb(sioaddr,0xe0,superio_inb(sioaddr,0xe0) | 0x40);
            superio_outb(sioaddr,0xe1,superio_inb(sioaddr,0xe1) & ~0x40);
            superio_outb(sioaddr,0xe3,superio_inb(sioaddr,0xe3) | 0x40);

            //GPIO Base IO port
#if defined(TS270) || defined(TS470) || defined(TS470U) || defined(TS670) || defined(TS870)
            //HDD Error LED:GP67
            superio_outb(sioaddr,0x90,(superio_inb(sioaddr,0x90) | 0x80));
            superio_outb(sioaddr,0x91,superio_inb(sioaddr,0x91) | 0x80 );
            superio_outb(sioaddr,0x93,superio_inb(sioaddr,0x93) | 0x80);
            //HDD Error LED:GP71-GP77
            superio_outb(sioaddr,0x80,(superio_inb(sioaddr,0x80) | 0xfe));
            superio_outb(sioaddr,0x81,superio_inb(sioaddr,0x81) | 0xfe );
            superio_outb(sioaddr,0x83,superio_inb(sioaddr,0x83) | 0xfe);
#else
            //10G NET LED: GP03
            superio_outb(sioaddr,0xf0,superio_inb(sioaddr,0xf0) | 0x08);
            superio_outb(sioaddr,0xf1,superio_inb(sioaddr,0xf1) | 0x08);
            superio_outb(sioaddr,0xf3,superio_inb(sioaddr,0xf3) | 0x08);

            //Power Detect:GP74,GP75
            //Board ID detect: GP70,GP71,PG72
            superio_outb(sioaddr,0x80,(superio_inb(sioaddr,0x80) & ~0x37));
#endif

#if defined(TS470U) // Support Digital IO Board
	    superio_outb(sioaddr,0xa0,superio_inb(sioaddr,0xa0) & ~0x0f);   // set GP50-53 input (GP50,51 Board ID), (GP52,53 PSU-OK)
	    superio_outb(sioaddr,0xc0,superio_inb(sioaddr,0xc0) & 0x00);    // set GP30-37 input (SIO DIN0-DIN7)
	    superio_outb(sioaddr,0xb0,superio_inb(sioaddr,0xb0) | 0x0c);    // set GP42-43 output (SIO DOUT0-DOUT1)
	    superio_outb(sioaddr,0xb3,superio_inb(sioaddr,0xb3) | 0x0c);    // set GP42-43 push-pull (SIO DOUT0-DOUT1)
	    superio_outb(sioaddr,0xb1,superio_inb(sioaddr,0xb1) & 0xf3);    // set GP42-43 output low
#endif

    //Patch by QNAP: always enable EVENT_INT# for WOL
            if (check_CMOS_initialized_by_BIOS() == 0)
            {
                superio_select(sioaddr, SIO_F71882FG_LD_PME);
                if (!(superio_inb(sioaddr,0x30) & 0x1))
                    superio_outb(sioaddr,0x30,1);
                superio_outb(sioaddr,0xe0,0x81);
                superio_outb(sioaddr,0xe1,superio_inb(sioaddr,0xe1) | 0xc0);
                superio_outb(sioaddr,0xe8,0x10);
                superio_outb(sioaddr,0xec,0x0);
            }
#endif  /*#if defined(X86_HASWELL) || defined(X86_BAYTRAIL) */
        }
#endif
//////////////////////////////////////////////////////
exit:
