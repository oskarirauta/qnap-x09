...
// QNAP Patch
extern int Is_Sata_7042(); //source code in sata_mv.c, do for hd serial number swap issue
...
// Fix bug#36179- HDD Smart->Hard Disk Infomation's Serial number display wong on 6281/6282/7042 controller
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_ARM)
#if defined(TS410) || defined(TS419)
	if(Is_Sata_7042())
#endif
	if(argbuf != NULL)
...
//		printk("=== check: call ata_cmd_ioctl: ATA_IDENTIFY_DEVICE, argsize = %d, dump all = %s\n",argsize,argbuf); // debug msg
//		printk("=== check: call ata_cmd_ioctl: ATA_IDENTIFY_DEVICE, dump sn = %s\n",(char*)(&argbuf[20])); // debug msg
		ata_id_string(argbuf,byte_buf, ATA_ID_SERNO, ATA_ID_SERNO_LEN);
//		printk("=== check: call ata_cmd_ioctl: ATA_IDENTIFY_DEVICE, dump sn = %s\n",byte_buf); // debug msg
		memcpy((char*)&word_raw[ATA_ID_SERNO],byte_buf,ATA_ID_SERNO_LEN);
//		printk("=== check3: call ata_cmd_ioctl: ATA_IDENTIFY_DEVICE, swap and dump sn = %s\n",(char*)(&argbuf[20])); // debug msg
	}
#endif
