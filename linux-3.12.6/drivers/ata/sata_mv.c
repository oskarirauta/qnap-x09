...
static char found_7042 = 0;
int Is_Sata_7042()
{
	return found_7042;
}
EXPORT_SYMBOL(Is_Sata_7042);
#endif
////////////////////////////////////////////////////////////////
...
static void mv_err_intr(struct ata_port *ap)
{
	...
	struct ata_queued_cmd *qc;
	int abort = 0;
//fix Bug#39117  fix the hardware issue
...
//fix Bug#39117  fix the hardware issue
			...
			if (!(edma_err_cause & (EDMA_ERR_DEV_DCON | EDMA_ERR_DEV_CON)))
			{
				...
				if(sd)
				{
					ssts=0x0;
					mdelay(130);
					sata_scr_read(&ap->link, SCR_STATUS, &ssts);
					if ((ssts & 0xf) != 0x3)
					{
						action |= ATA_EH_RESET;
						ata_ehi_hotplugged(ehi);
...
#endif //fix Bug#39117  fix the hardware issue
		}
	}
...
#if defined(CONFIG_MACH_QNAPTS)
// QNAP Patch: fix Bug #27328, Solution from Marvell Patch on 2013.07.15
	reg &= ~(0x3 << 23);    /* SELMUFF (bits 24:23) to 1 */
	reg |= (0x1 << 23);
	reg &= ~(0x3 << 25);    /* SELMUPI (bits 26:25) to 1 */
	reg |= (0x1 << 25);
//////// QNAP Patch End
#endif
