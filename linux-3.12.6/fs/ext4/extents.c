...
long ext4_fallocate(struct file *file, int mode, loff_t offset, loff_t len)
{
	...
	if (mode & ~(FALLOC_FL_KEEP_SIZE | FALLOC_FL_PUNCH_HOLE | FALLOC_FL_NO_HIDE_STALE))
		return -EOPNOTSUPP;
...
retry:
	while (ret >= 0 && ret < max_blocks) {
		...
		if (q && q->thindata && !blk_queue_thick(q) && !thin_clone){ /* Only thin device should care about this */
...
#define EXT4_FIEMAP_FLAGS	(FIEMAP_FLAG_SYNC|FIEMAP_FLAG_XATTR|FIEMAP_FLAG_KERNEL)
