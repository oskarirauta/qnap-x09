...
static int ext4_dentry_revalidate(struct dentry *dentry, unsigned int nd)
{
	if(nd & LOOKUP_CASE_INSENSITIVE && dentry && dentry->d_inode == NULL){
...
#endif
{
	...
	return search_dir(bh, bh->b_data, dir->i_sb->s_blocksize, dir,
			  d_name, offset, res_dir,case_sensitive);
#else
	return search_dir(bh, bh->b_data, dir->i_sb->s_blocksize, dir,
...
static struct buffer_head * ext4_find_entry (struct inode *dir,
					const struct qstr *d_name,
					struct ext4_dir_entry_2 **res_dir,
					int *inlined, int case_sensitive)
#else
static struct buffer_head * ext4_find_entry (struct inode *dir,
					const struct qstr *d_name,
					struct ext4_dir_entry_2 **res_dir,
	...
	*res_dir = NULL;
	...
	if (ext4_has_inline_data(dir)) {
		...
		ret = ext4_find_inline_entry(dir, d_name, res_dir,
					     &has_inline_data, case_sensitive);
#else
		ret = ext4_find_inline_entry(dir, d_name, res_dir,
					     &has_inline_data);
#endif
...
#ifdef CONFIG_MACH_QNAPTS
#ifdef QNAP_SEARCH_FILENAME_CASE_INSENSITIVE
	bh = ext4_find_entry(dir, &dentry->d_name, &de, NULL, case_sensitive);
#ifdef CASE_FOLDING_DEBUG
	if (debug)
...
#ifdef CONFIG_MACH_QNAPTS
#ifdef QNAP_SEARCH_FILENAME_CASE_INSENSITIVE
	bh = ext4_find_entry(dir, &dentry->d_name, &de, NULL, 1);
#ifdef CASE_FOLDING_DEBUG
	if (debug)
...
#ifdef CONFIG_MACH_QNAPTS
#ifdef QNAP_SEARCH_FILENAME_CASE_INSENSITIVE
	old_bh = ext4_find_entry(old_dir, &old_dentry->d_name, &old_de, NULL, 1);
#else
